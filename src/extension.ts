...
export function activate(
	...
	omitLanguages = config.get("omit", [
		"plaintext",
	]);

	let blockHighlighter = new BlockHL();
	let blockHighlighterController =
		new BHLController(blockHighlighter);

	blockHighlighter.updateLine();
	context.subscriptions.push(blockHighlighter);
	context.subscriptions.push(
		blockHighlighterController,
	);
}
...
class BlockHL {
	public updateLine() {
		...
		if (editor.selection.isSingleLine) {
			...
			if (
				...
				botLine = this.pruneTrailingWhitespace(
					editor,
					botLine,
				);
			}
		...
		console.log(
			"Update Time: ".concat(String(end - start)),
		);
		console.log(
			"Selected Line Level: " +
				this.getIndentLevel(
	...
	findTop(editor: vscode.TextEditor) {
		...
		if (
			...
			let nextLine = editor.document.lineAt(
				line.lineNumber + 1,
			);
			...
			while (
				...
				nextLine = editor.document.lineAt(
		...
		while (line.lineNumber > 0) {
			if (!line.isEmptyOrWhitespace) {
				let nextLevel = this.getIndentLevel(
					editor,
					line,
				);
	...
	pruneTrailingWhitespace(
		...
		let newBot = editor.document.lineAt(
			bot.lineNumber - 1,
		);
		while (newBot.isEmptyOrWhitespace) {
			newBot = editor.document.lineAt(
				newBot.lineNumber - 1,
			);
		}
	...
	setCurrentDocumentTabSize() {
		...
		TAB_SIZE = tabs;
		console.log(
			"Tab size of current document: " + TAB_SIZE,
		);
	}
	...
	highlightRange(
		...
		let wholeLine: boolean = config.get(
			"isWholeLine",
			true,
		);
		let accentCurrent: boolean = config.get(
			"accentCurrentLine",
			true,
		);
		let preferThemeColor: boolean = config.get(
			"preferThemeColor",
			true,
		);
		...
		const highlightColor = new vscode.ThemeColor(
			"blockPopper.highlight",
		);
		...
		highlightDecoration =
			vscode.window.createTextEditorDecorationType(
				<vscode.DecorationRenderOptions>{
					backgroundColor: preferThemeColor
						? highlightColor
						: backgroundColor,
